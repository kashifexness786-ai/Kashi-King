...
int OnInit()
{
   Print("AtifKhan EA initializing...");
   DetectSymbols();
...
void ProcessSymbol(const string symbol, Sequence &seq)
{
   ...
   ulong pos_ticket=0; long pos_type=-1; bool hasPos = HasOpenPosition(symbol, InpMagicNumber, pos_ticket, pos_type);
   if(prevBullish && currentMovedBelowOpen && touchedPrevLow)
   {
      if(!hasPos)
      {
         ...
         if(ExecuteSell(symbol, InpLotSize, sl, tp)) { seq.state=SEQ_SELL; seq.candle_time=curM5; seq.sequence_end=curM5+InpSequenceMinutes*60; seq.candle_open=open0; seq.last_sl=sl; seq.last_tp=tp; seq.active=true; PrintFormat("SELL sequence started on %s",symbol); }
      }
   }
   ...
   if(buyCondition && !hasPos)
   {
      ...
      if(ExecuteBuy(symbol, InpLotSize, sl, tp)) { seq.state=SEQ_BUY; seq.candle_time=curM5; seq.sequence_end=curM5+InpSequenceMinutes*60; seq.candle_open=open0; seq.last_sl=sl; seq.last_tp=tp; seq.active=true; PrintFormat("BUY sequence started on %s",symbol); }
   }
   if(seq.active)
   {
